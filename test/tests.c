...
void insert_test_data(uint16_t address, uint8_t data)
{
    ...
    mos6502_get_data_bus(&current_data);
    mos6502_set_data_bus(data);
    mos6502_set_address_bus(address);
    memmap_write();
    mos6502_set_data_bus(current_data);
...
void test_LDA_Immediate(void)
{
    ...
    /* Perform test */
    program_load(test_program_LDA_Immediate);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the immediate operand */
    /* End test */
...
void test_LDA_Zero_Page(void)
{
    ...
    program_load(test_program_LDA_Zero_Page);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
    /* End test */
...
void test_LDA_Zero_Page_X_Indexed(void)
{
    ...
    /* The program specifies 0x90, this offset gets us to the correct location */
    mos6502_set_register(MOS6502_REG_X, 0x01);

    /* Now load our test program and start clocking the CPU */
    program_load(test_program_LDA_Zero_Page_X_Indexed);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and use it as the next address */
    mos6502_clock_tick(); /* Fetch the X register and index it with the address */
    /* End test */
...
void test_LDA_Absolute(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_LDA_Absolute);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the low memory location */
    mos6502_clock_tick(); /* Fetch the next byte for the high memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
    /* End test */
...
void test_LDA_Absolute_X_Indexed(void)
{
    ...
    /* The program specifies 0x2000, this offset gets us to the correct location */
    mos6502_set_register(MOS6502_REG_X, 0x01);

    /* Now load our test program and start clocking the CPU */
    program_load(test_program_LDA_Absolute_X_Indexed);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the low memory location */
    mos6502_clock_tick(); /* Fetch the next byte for the high memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
...
void test_LDA_Absolute_X_Indexed_Boundary_Cross(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_LDA_Absolute_X_Indexed_Boundary_Cross);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the low memory location */
    mos6502_clock_tick(); /* Fetch the next byte for the high memory location */
    mos6502_clock_tick(); /* Compute new address with carry */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
    /* End test */
...
void test_LDA_Absolute_Y_Indexed(void)
{
    ...
    /* The program specifies 0x2000, this offset gets us to the correct location */
    mos6502_set_register(MOS6502_REG_Y, 0x01);

    /* Now load our test program and start clocking the CPU */
    program_load(test_program_LDA_Absolute_Y_Indexed);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the low memory location */
    mos6502_clock_tick(); /* Fetch the next byte for the high memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
...
void test_LDA_Absolute_Y_Indexed_Boundary_Cross(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_LDA_Absolute_Y_Indexed_Boundary_Cross);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the low memory location */
    mos6502_clock_tick(); /* Fetch the next byte for the high memory location */
    mos6502_clock_tick(); /* Compute new address with carry */
...
void test_LDA_Indirect_X_Indexed(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_LDA_Indirect_X_Indexed);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the zero page base address */
    mos6502_clock_tick(); /* Set the data bus */
    mos6502_clock_tick(); /* Apply X offset and fetch low byte */
    mos6502_clock_tick(); /* Fetch high byte */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
    /* End test */
...
void test_LDA_Indirect_Y_Indexed(void)
{
    ...
    mos6502_set_register(MOS6502_REG_Y, 0x02);

    /* Now load our test program and start clocking the CPU */
    program_load(test_program_LDA_Indirect_Y_Indexed);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the address offset */
    mos6502_clock_tick(); /* Apply the offset and read the low byte */
    mos6502_clock_tick(); /* Increment address and read high byte */
    mos6502_clock_tick(); /* Apply Y index offset */
    mos6502_clock_tick(); /* Fetch data */
    /* End test */
...
void test_LDX_Immediate(void)
{
    ...
    /* Perform test */
    program_load(test_program_LDX_Immediate);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the immediate operand */
    /* End test */
...
void test_LDX_Zero_Page(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_LDX_Zero_Page);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
    /* End test */
...
void test_LDX_Zero_Page_Y_Indexed(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_LDX_Zero_Page_Y_Indexed);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and use it as the next address */
    mos6502_clock_tick(); /* Fetch the X register and index it with the address */
...
void test_LDX_Absolute(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_LDX_Absolute);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the low memory location */
    mos6502_clock_tick(); /* Fetch the next byte for the high memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
...
void test_LDX_Absolute_Y_Indexed(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_LDX_Absolute_Y_Indexed);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the low memory location */
    mos6502_clock_tick(); /* Fetch the next byte for the high memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
...
void test_LDX_Absolute_Y_Indexed_Boundary_Cross(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_LDX_Absolute_Y_Indexed_Boundary_Cross);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the low memory location */
    mos6502_clock_tick(); /* Fetch the next byte for the high memory location */
    mos6502_clock_tick(); /* Compute new address with carry */
...
void test_LDY_Immediate(void)
{
    ...
    /* Perform test */
    program_load(test_program_LDY_Immediate);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the immediate operand */
    /* End test */
...
void test_LDY_Zero_Page(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_LDY_Zero_Page);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
    /* End test */
...
void test_LDY_Zero_Page_X_Indexed(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_LDY_Zero_Page_X_Indexed);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and use it as the next address */
    mos6502_clock_tick(); /* Fetch the X register and index it with the address */
...
void test_LDY_Absolute(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_LDY_Absolute);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the low memory location */
    mos6502_clock_tick(); /* Fetch the next byte for the high memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
...
void test_LDY_Absolute_X_Indexed(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_LDY_Absolute_X_Indexed);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the low memory location */
    mos6502_clock_tick(); /* Fetch the next byte for the high memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
...
void test_LDY_Absolute_X_Indexed_Boundary_Cross(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_LDY_Absolute_X_Indexed_Boundary_Cross);
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the low memory location */
    mos6502_clock_tick(); /* Fetch the next byte for the high memory location */
    mos6502_clock_tick(); /* Compute new address with carry */
...
void test_STA_Zero_Page(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_STA_Zero_Page);
    ...
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Store the Accumulator value into memory */
    /* End test */
...
void test_STA_Zero_Page_X_Indexed(void)
{
    ...
    /* Set out offset to be added to the address in the program */
    mos6502_set_register(MOS6502_REG_X, 0x09);

    /* Now load our test program and start clocking the CPU */
    program_load(test_program_STA_Zero_Page_X_Indexed);
    /* Load (LDA) a value into the Accumulator */
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
    ...
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Update the address bus to the memory address + index */
    mos6502_clock_tick(); /* Store the Accumulator value into memory */
    /* End test */
...
void test_STA_Absolute(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_STA_Absolute);
    /* Load (LDA) a value into the Accumulator */
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
    ...
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Update the address bus to the memory address */
    mos6502_clock_tick(); /* Store the Accumulator value into memory */
    /* End test */
...
void test_STA_Absolute_X_Indexed(void)
{
    ...
    /* Set out offset to be added to the address in the program */
    mos6502_set_register(MOS6502_REG_X, 0x0F);

    /* Now load our test program and start clocking the CPU */
    program_load(test_program_STA_Absolute_X_Indexed);
    /* Load (LDA) a value into the Accumulator */
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
...
void test_STA_Absolute_Y_Indexed(void)
{
    ...
    /* Set out offset to be added to the address in the program */
    mos6502_set_register(MOS6502_REG_Y, 0x0F);

    /* Now load our test program and start clocking the CPU */
    program_load(test_program_STA_Absolute_Y_Indexed);
    /* Load (LDA) a value into the Accumulator */
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
...
void test_STA_Indirect_X_Indexed(void)
{
    ...
    /* Set out offset to be added to the address in the program */
    mos6502_set_register(MOS6502_REG_X, 0x05);

    /* Now load our test program and start clocking the CPU */
    program_load(test_program_STA_Indirect_X_Indexed);
    /* Load (LDA) a value into the Accumulator */
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
...
void test_STA_Indirect_Y_Indexed(void)
{
    ...
    /* Set out offset to be added to the address in the program */
    mos6502_set_register(MOS6502_REG_Y, 0x05);

    /* Now load our test program and start clocking the CPU */
    program_load(test_program_STA_Indirect_Y_Indexed);
    /* Load (LDA) a value into the Accumulator */
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
...
void test_STX_Zero_Page(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_STX_Zero_Page);
    ...
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Store the X index value into memory */
    /* End test */
...
void test_STX_Zero_Page_Y_Indexed(void)
{
    ...
    /* Set out offset to be added to the address in the program */
    mos6502_set_register(MOS6502_REG_Y, 0x09);

    /* Now load our test program and start clocking the CPU */
    program_load(test_program_STX_Zero_Page_Y_Indexed);
    /* Load (LDX) a value into the X index register */
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
...
void test_STX_Absolute(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_STX_Absolute);
    /* Load (LDX) a value into the X index register */
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
...
void test_STY_Zero_Page(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_STY_Zero_Page);
    ...
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Store the Y index value into memory */
    /* End test */
...
void test_STY_Zero_Page_X_Indexed(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_STY_Zero_Page_X_Indexed);
    /* Load (LDY) a value into the Y index register */
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
...
void test_STY_Absolute(void)
{
    ...
    /* Now load our test program and start clocking the CPU */
    program_load(test_program_STY_Absolute);
    /* Load (LDY) a value into the Y index register */
    mos6502_clock_tick(); /* Read the instruction */
    mos6502_clock_tick(); /* Fetch the next byte for the memory location */
    mos6502_clock_tick(); /* Fetch the value from memory and load it */
...
void test_ADC_Immediate(void)
{
    ...
    mos6502_get_register(MOS6502_REG_S, &P);
    assert(MOS6502_STATUS_FLAG_CARRY & P);

    mos6502_get_register(MOS6502_REG_A, &accumulator);
