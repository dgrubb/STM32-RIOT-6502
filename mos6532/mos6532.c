 ...
 */
inline int mos6532_bounds_check(uint8_t address)
{
    if (address > MEM_SIZE) {
        return -1;
    }
 ...
 * Returns 0 on success, -1 on error.
 */
inline int mos6532_read(uint16_t address, uint8_t *data)
{
 ...
 * Returns 0 on success, -1 on error.
 */
inline int mos6532_write(uint16_t address,uint8_t data)
{
    if (-1 == mos6532_bounds_check(address)) {
        /* Error, attempting to write outside memory */
        return -1;
